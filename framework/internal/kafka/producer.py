...
class Producer(Singleton):
    ...
    def send(self, topic: str, message: dict[str, str]) -> None:
        if not self._producer:
            raise RuntimeError('Producer must be started before sending message')
        try:
            with self._lock:
    ...
    def __exit__(self, exc_type: type[BaseException], exc_val: BaseException | None, exc_tb: TracebackType) -> None:
        self.stop()
