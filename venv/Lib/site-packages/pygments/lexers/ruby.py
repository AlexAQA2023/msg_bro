...
class RubyLexer(ExtendedRegexLexer):
    ...
    def heredoc_callback(self, match, ctx):
        ...
        start = match._start(1)
        yield start, Operator, match.group(1)        # <<[-~]?
        yield match._start(2), String.Heredoc, match.group(2)   # quote ", ', `
        yield match._start(3), String.Delimiter, match.group(3) # heredoc name
        yield match._start(4), String.Heredoc, match.group(4)   # quote again

        heredocstack = ctx.__dict__.setdefault('heredocstack', [])
        outermost = not bool(heredocstack)
        heredocstack.append((match.group(1) in ('<<-', '<<~'), match.group(3)))

        ctx.pos = match._start(5)
        ctx.end = match.end(5)
        ...
        if outermost:
            ...
            for tolerant, hdname in heredocstack:
                ...
                for match in line_re.finditer(ctx.text, ctx.pos):
                    ...
                    if check == hdname:
                        for amatch in lines:
                            yield amatch._start(), String.Heredoc, amatch.group()
                        yield match._start(), String.Delimiter, match.group()
                        ctx.pos = match.end()
                        break
                    else:
                        lines.append(match)
                    ...
                    for amatch in lines:
                        yield amatch._start(), Error, amatch.group()
            ctx.end = len(ctx.text)
            del heredocstack[:]

    def gen_rubystrings_rules():
        def intp_regex_callback(self, match, ctx):
            yield match._start(1), String.Regex, match.group(1)  # begin
            nctx = LexerContext(match.group(3), 0, ['interpolated-regex'])
            for i, t, v in self.get_tokens_unprocessed(context=nctx):
                yield match._start(3) + i, t, v
            yield match._start(4), String.Regex, match.group(4)  # end[mixounse]*
            ctx.pos = match.end()

        def intp_string_callback(self, match, ctx):
            yield match._start(1), String.Other, match.group(1)
            nctx = LexerContext(match.group(3), 0, ['interpolated-string'])
            for i, t, v in self.get_tokens_unprocessed(context=nctx):
                yield match._start(3) + i, t, v
            yield match._start(4), String.Other, match.group(4)  # end
            ctx.pos = match.end()
        ...
        for match in line_re.finditer(text):
            ...
            if m is not None:
                ...
                if curcode:
                    ...
                    curcode = ''
                    insertions = []
                yield match._start(), Generic.Output, line
        if curcode:
            yield from do_insertions(
                insertions, rblexer.get_tokens_unprocessed(curcode))
