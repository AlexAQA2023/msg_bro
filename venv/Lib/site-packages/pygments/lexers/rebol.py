...
class RebolLexer(RegexLexer):
    ...
    def word_callback(lexer, match):
        ...
        if re.match(".*:$", word):
            yield match._start(), Generic.Subheading, word
        elif re.match(
            ...
            r'rsa-encrypt)$', word):
            yield match._start(), Name.Builtin, word
        elif re.match(
            ...
            r'copy)$', word):
            yield match._start(), Name.Function, word
        elif re.match(
            ...
            yield match._start(), Keyword.Namespace, word
        elif re.match(
            r'(halt|quit|do|load|q|recycle|call|run|ask|parse|view|unview|'
            r'return|exit|break)$', word):
            yield match._start(), Name.Exception, word
        elif re.match('REBOL$', word):
            yield match._start(), Generic.Heading, word
        elif re.match("to-.*", word):
            yield match._start(), Keyword, word
        elif re.match(r'(\+|-|\*|/|//|\*\*|and|or|xor|=\?|=|==|<>|<|>|<=|>=)$',
                      word):
            yield match._start(), Operator, word
        elif re.match(r".*\?$", word):
            yield match._start(), Keyword, word
        elif re.match(r".*\!$", word):
            yield match._start(), Keyword.Type, word
        elif re.match("'.*", word):
            yield match._start(), Name.Variable.Instance, word  # lit-word
        elif re.match("#.*", word):
            yield match._start(), Name.Label, word  # issue
        elif re.match("%.*", word):
            yield match._start(), Name.Decorator, word  # file
        else:
            yield match._start(), Name.Variable, word
...
class RedLexer(RegexLexer):
    ...
    def word_callback(lexer, match):
        ...
        if re.match(".*:$", word):
            ...
            yield match._start(), Name.Builtin.Pseudo, word
            ...
            yield match._start(), Generic.Subheading, word  # get-word
        else:
            yield match._start(), Name.Variable, word
