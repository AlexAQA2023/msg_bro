...
class MIMELexer(RegexLexer):
    ...
    def get_header_tokens(self, match):
        ...
        if field.lower() in self.attention_headers:
            yield match._start(1), Name.Tag, field + ":"
            yield match._start(2), Text.Whitespace, match.group(2)
            ...
            yield match._start(), Comment, match.group()

    def get_body_tokens(self, match):
        pos_body_start = match._start()
    ...
    def store_content_type(self, match):
        ...
        prefix_len = match._start(1) - match._start(0)
        yield match._start(0), Text.Whitespace, match.group(0)[:prefix_len]
        yield match._start(1), Name.Label, match.group(2)
        yield match.end(2), String.Delimiter, '/'
        yield match._start(3), Name.Label, match.group(3)

    def get_content_type_subtokens(self, match):
        yield match._start(1), Text, match.group(1)
        yield match._start(2), Text.Whitespace, match.group(2)
        yield match._start(3), Name.Attribute, match.group(3)
        yield match._start(4), Operator, match.group(4)
    ...
    def store_content_transfer_encoding(self, match):
        ...
        yield match._start(0), Name.Constant, match.group(0)
