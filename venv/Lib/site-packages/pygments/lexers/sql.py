...
def language_callback(lexer, match):
    ...
    if m is not None:
        ...
        m = list(language_re.finditer(
            lexer.text[max(0, match._start() - 100):match._start()]))
        if m:
            lx = lexer._get_lexer(m[-1].group(1))
        else:
            m = list(do_re.finditer(
                lexer.text[max(0, match._start() - 25):match._start()]))
            if m:
                lx = lexer._get_lexer('plpgsql')

    # 1 = $, 2 = delimiter, 3 = $
    yield (match._start(1), String, match.group(1))
    yield (match._start(2), String.Delimiter, match.group(2))
    yield (match._start(3), String, match.group(3))
    # 4 = string contents
    if lx:
    ...
    yield (match._start(6), String.Delimiter, match.group(6))
    yield (match._start(7), String, match.group(7))
