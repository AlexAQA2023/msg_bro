...
class XQueryLexer(ExtendedRegexLexer):
    ...
    def punctuation_root_callback(lexer, match, ctx):
        yield match._start(), Punctuation, match.group(1)
        # transition to root always - don't pop off stack
        ctx.stack = ['root']
        ctx.pos = match.end()

    def operator_root_callback(lexer, match, ctx):
        yield match._start(), Operator, match.group(1)
        # transition to root always - don't pop off stack
        ctx.stack = ['root']
        ctx.pos = match.end()

    def popstate_tag_callback(lexer, match, ctx):
        yield match._start(), Name.Tag, match.group(1)
        if lexer.xquery_parse_state:
            ctx.stack.append(lexer.xquery_parse_state.pop())
        ctx.pos = match.end()

    def popstate_xmlcomment_callback(lexer, match, ctx):
        yield match._start(), String.Doc, match.group(1)
        ctx.stack.append(lexer.xquery_parse_state.pop())
        ctx.pos = match.end()
    ...
    def popstate_kindtest_callback(lexer, match, ctx):
        ...
        if next_state == 'occurrenceindicator':
            if re.match("[?*+]+", match.group(2)):
                yield match._start(), Punctuation, match.group(2)
    ...
    def pushstate_operator_order_callback(lexer, match, ctx):
        yield match._start(), Keyword, match.group(1)
        yield match._start(), Whitespace, match.group(2)
        yield match._start(), Punctuation, match.group(3)
        ctx.stack = ['root']
    ...
    def pushstate_operator_root_validate_withmode(lexer, match, ctx):
        yield match._start(), Keyword, match.group(1)
        yield match._start(), Whitespace, match.group(2)
        yield match._start(), Keyword, match.group(3)
        ctx.stack = ['root']
        lexer.xquery_parse_state.append('operator')
        ctx.pos = match.end()
    ...
    def pushstate_operator_attribute_callback(lexer, match, ctx):
        yield match._start(), Name.Attribute, match.group(1)
        ctx.stack.append('operator')
        ctx.pos = match.end()
