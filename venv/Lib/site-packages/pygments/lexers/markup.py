...
class RstLexer(RegexLexer):
    ...
    def _handle_sourcecode(self, match):
        ...
        # section header
        yield match._start(1), Punctuation, match.group(1)
        yield match._start(2), Text, match.group(2)
        yield match._start(3), Operator.Word, match.group(3)
        yield match._start(4), Punctuation, match.group(4)
        yield match._start(5), Text, match.group(5)
        yield match._start(6), Keyword, match.group(6)
        yield match._start(7), Text, match.group(7)
        ...
        if lexer is None:
            yield match._start(8), String, code
            return
...
class MarkdownLexer(RegexLexer):
    ...
    def _handle_codeblock(self, match):
        ...
        yield match._start('initial'), String.Backtick, match.group('initial')
        yield match._start('lang'), String.Backtick, match.group('lang')
        if match.group('afterlang') is not None:
            yield match._start('whitespace'), Whitespace, match.group('whitespace')
            yield match._start('extra'), Text, match.group('extra')
        yield match._start('newline'), Whitespace, match.group('newline')
        ...
        # no lexer for this language. handle it like it was a code block
        if lexer is None:
            yield match._start('code'), String, code
        else:
            # FIXME: aren't the offsets wrong?
            yield from do_insertions([], lexer.get_tokens_unprocessed(code))

        yield match._start('terminator'), String.Backtick, match.group('terminator')
...
class TiddlyWiki5Lexer(RegexLexer):
    ...
    def _handle_codeblock(self, match):
        ...
        # section header
        yield match._start(1), String, match.group(1)
        yield match._start(2), String, match.group(2)
        yield match._start(3), Text,   match.group(3)
        ...
        # no lexer for this language. handle it like it was a code block
        if lexer is None:
            yield match._start(4), String, code
            return

        yield from do_insertions([], lexer.get_tokens_unprocessed(code))

        yield match._start(5), String, match.group(5)
    ...
    def _handle_cssblock(self, match):
        ...
        # no lexer for this language. handle it like it was a code block
        if lexer is None:
            yield match._start(3), String, code
            return

        yield from do_insertions([], lexer.get_tokens_unprocessed(code))

        yield match._start(4), String, match.group(4)
...
class WikitextLexer(RegexLexer):
    ...
    def handle_syntaxhighlight(self, match, ctx):
        ...
        yield match._start(3) + index, Punctuation, '>'
        ...
        if lexer is None:
            yield match._start() + index + 1, Text, content
        else:
